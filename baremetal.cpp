...
// Bring the GPIO hardware blocks out of reset.
// On RP2350, hardware blocks start in reset after power-up.
// A reset bit value of 1 means "keep this hardware block reset".
// A reset bit value of 0 means "let this hardware block run".
void init_gpio_hardware() {
    // Combine the two reset bits we care about into one mask.
    // | is bitwise OR: it makes a value with both bits set.
    const uint32_t gpio_reset_bits = RESET_IO_BANK0 | RESET_PADS_BANK0;

    // Read the current RESET register, clear only our two GPIO-related bits,
    // then write the updated value back.
    // ~ flips the mask bits, so & ~gpio_reset_bits keeps every other reset bit
    // unchanged while clearing IO_BANK0 and PADS_BANK0.
    uint32_t reset_value = mmio_read(RESETS_RESET);
    reset_value = reset_value & ~gpio_reset_bits;
    mmio_write(RESETS_RESET, reset_value);

    // Wait until both hardware blocks report that they are out of reset.
    // This loop repeatedly reads RESET_DONE until both desired bits are 1.
    while ((mmio_read(RESETS_RESET_DONE) & gpio_reset_bits) != gpio_reset_bits) {
        // Empty on purpose: the CPU waits here until the hardware is ready.
    }
}
...
int main() {
    ...
    // This calls init_gpio_hardware(), defined above.
    init_gpio_hardware();
